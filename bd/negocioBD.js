...
async function mostrarNegocio() {
    ...
    try{
        ...
        console.log("Error al obtener la compra de firebase "+err);
        neg.push(null);
    }
    return neg;
}
...
async function buscarNegocioID(id){
    ...
    try{
        ...
        console.log("Error al buscar la compra"+err);
    }
    return neg;
}
...
async function nuevoNegocio(datos){
    var negocio=new Negocio(null,datos);
    var error=1;
    if(negocio.bandera==0){
        try{
            await conexion.doc().set(negocio.obtenerDatos);
...
async function modificarNegocio(datos){
    ...
    if(negocio!=undefined){  
        ...
        if (datos.identificacion !== undefined && datos.identificacion !== negocio.identificacion) {
            var fotoRuta = './web/Negocios/images/' + negocio.identificacion;
       ...
       if(negocio.bandera==0){
           try{
               ...
               console.log("Error al modificar la compra"+err);
           }
       }else{
...
async function borrarNegocio(id){
    ...
    if (negocio != undefined) {
        ...
        try{
            ...
            console.log("Error al borrar compra: "+err);
        }
        return error;     
    }
}
